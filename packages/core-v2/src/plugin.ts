...
import { type ColorSchema, generateColorTheme, generateColorSemanticVariables } from './colors.ts'
 ...
 */
export const natcorePlugin: ReturnType<typeof plugin.withOptions<NatcorePluginOptions>> = plugin.withOptions<NatcorePluginOptions>(
  (options = {}) => {
    ...
    return function({ addBase, matchUtilities, theme }) {
      ...
      Object.entries(colors).forEach(([name, colorConfig]) => {
        const baseColor = typeof colorConfig === 'string' ? colorConfig : colorConfig.color
        const baseShade = typeof colorConfig === 'string' ? 500 : colorConfig.shade
        
        const shades = [50, 100, 200, 300, 400, 500, 600, 700, 800, 900]
        const shadeToLightness: Record<number, number> = {
          50: 0.97, 100: 0.93, 200: 0.85, 300: 0.75, 400: 0.65,
          500: 0.55, 600: 0.45, 700: 0.35, 800: 0.25, 900: 0.15,
        }
        ...
        shades.forEach(shade => {
          ...
          const adjustment = diff === 0 ? '' : diff > 0 ? ` + ${diff.toFixed(2)}` : ` - ${Math.abs(diff).toFixed(2)}`
          
          colorVariables[`--color-${shade}-bg-${name}`] = `oklch(from ${baseColor} calc(l${adjustment}) c h)`
          colorVariables[`--color-${shade}-fg-${name}`] = `oklch(from var(--color-${shade}-bg-${name}) calc((l - 0.6) * -100) 0 h)`
        })
      })
