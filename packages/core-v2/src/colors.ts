 ...
 */
export function generateColorVariables(colorName: string, baseColor: string, baseShade: number = 500): Record<string, string> {
  ...
  shades.forEach(shade => {
    ...
    variables[`--color-${colorName}-${shade}`] = `oklch(from var(--color-${colorName}-base) calc(l ${lightnessAdjust}) c h)`;
    variables[`--color-${colorName}-${shade}-rgb`] = `srgb(from var(--color-${colorName}-${shade}) r g b)`;
    
    // Generate contrasting text color (uses OKLCH relative lightness)
    // If color is light (L > 0.6), use dark text, otherwise light text
    variables[`--color-${colorName}-${shade}-text`] = `oklch(from var(--color-${colorName}-${shade}) calc((l - 0.6) * -100) 0 h)`;
    variables[`--color-${colorName}-${shade}-text-rgb`] = `srgb(from var(--color-${colorName}-${shade}-text) r g b)`;
  });
 ...
 */
function calculateLightnessAdjust(targetShade: number, baseShade: number): string {
 ...
 */
export function generateColorSemanticVariables(colors: ColorSchema): Record<string, string> {
  const variables: Record<string, string> = {};
  
  Object.keys(colors).forEach(colorName => {
    Object.entries(VARIABLES_TO_SHADES).forEach(([varName, shade]) => {
      variables[`--${colorName}-${varName}`] = `var(--color-${colorName}-${shade})`;
      variables[`--${colorName}-${varName}-text`] = `var(--color-${colorName}-${shade}-text)`;
    });
  });
