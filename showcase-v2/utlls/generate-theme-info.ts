...
export type ThemeFactory = (theme: PluginAPI['theme']) => Readonly<Record<string, any>>

type RelevantClassKeys<ObjectType extends object> = {
  [Key in keyof ObjectType & string]: Key extends `${string}.${string}` | `&.${string}`
    ...
    for (const key in obj) {
      ...
      for (const c of potentialThemeClass) {
        if (
          ...
          const className = c.slice(type === 'component' ? c.indexOf('.') + 1 : 2)
