...
export function getTrayPlaygroundShowcase(values: TrayControlValues) {
  ...
  const shellClassName = twMerge(
    'relative w-full overflow-hidden rounded-xl bg-tone-100-surface/25',
    isInline ? 'flex min-h-64 gap-4 p-4' : 'h-64'
  )
  const workspaceClassName = twMerge('min-w-0 p-5', isInline ? 'order-2 flex-1' : 'h-full')
  const trayClassName = twMerge(
    ...
    'bg-tone-50-surface p-5 shadow-xl',
    ...
    <ui-div className={shellClassName}>
      <div className={workspaceClassName}>
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-sm font-semibold text-on-tone-50-surface">Workspace</p>
            <p className="text-xs text-on-tone-50-surface/60">
              {isInline
          ...
          <label htmlFor={toggleId} className="btn-solid/primary cursor-pointer btn-size-sm">
            Toggle tray
          </label>
        </div>

        <div className="mt-6 grid gap-3">
          <div className="h-3 w-1/2 rounded-full bg-tone-200-surface/70" />
          <div className="h-3 w-4/5 rounded-full bg-tone-200-surface/50" />
          <div className="h-3 w-2/3 rounded-full bg-tone-200-surface/40" />
        </div>
  ...
  return (
    <div className="flex h-full flex-col gap-4">
      <div>
        ...
        <p className="font-mono text-xs text-on-tone-50-surface/55">tray-{direction}</p>
      </div>

      <div className="space-y-2 text-xs text-on-tone-50-surface/70">
        <div className="rounded-md bg-tone-100-surface/60 px-3 py-2">Navigation</div>
        <div className="rounded-md bg-tone-100-surface/60 px-3 py-2">Filters</div>
        <div className="rounded-md bg-tone-100-surface/60 px-3 py-2">Settings</div>
      </div>
    </div>
  )
}
...
function ContentTile({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-md bg-tone-50-surface/45 p-3">
      <p className="text-xs text-on-tone-50-surface/55">{label}</p>
      <p className="text-sm font-semibold text-on-tone-50-surface">{value}</p>
    </div>
  )
}
