...
export default {
  ...
  atAGlance: (
    <ui-div className="md:grid-cols-2 grid w-full gap-4">
      <TrayFrame>
        <div className="tray-left w-44 tray-open bg-tone-50-surface p-4 shadow-lg">
          <TrayContent title="Left tray" />
        </div>
      </TrayFrame>
      <TrayFrame>
        <div className="tray-bottom h-24 tray-open bg-tone-50-surface p-4 shadow-lg">
  ...
  return (
    <ui-div className="md:grid-cols-2 grid w-full gap-4">
      <TrayFrame label="tray-left">
        <div className="tray-left w-40 tray-open bg-tone-50-surface p-4 shadow-lg">
          <TrayContent title="Left" />
        </div>
      </TrayFrame>
      <TrayFrame label="tray-right">
        <div className="tray-right w-40 tray-open bg-tone-50-surface p-4 shadow-lg">
          <TrayContent title="Right" />
        </div>
      </TrayFrame>
      <TrayFrame label="tray-top">
        <div className="tray-top h-24 tray-open bg-tone-50-surface p-4 shadow-lg">
...
function inlineShowcase() {
  return (
    <ui-div className="flex w-full gap-4 rounded-xl border border-tone-300-surface/40 bg-tone-100-surface/25 p-4">
      <div className="tray-inline w-48 shrink-0 rounded-lg bg-tone-50-surface p-4 shadow-sm">
        <div className="mb-3 font-mono text-xs text-on-tone-50-surface/55">tray-inline</div>
        <TrayContent title="Tray" />
      </div>
      <div className="min-w-0 flex-1 rounded-lg bg-tone-50-surface/45 p-4">
        <p className="text-sm font-semibold text-on-tone-50-surface">Content</p>
        <p className="mt-2 text-xs text-on-tone-50-surface/60">
          The inline tray stays beside this content in normal document flow.
        </p>
        <div className="mt-4 space-y-2">
          <div className="h-2.5 w-3/4 rounded-full bg-tone-200-surface/65" />
          <div className="h-2.5 w-1/2 rounded-full bg-tone-200-surface/45" />
        </div>
      </div>
    </ui-div>
  )
  ...
  return (
    <ui-div className="md:grid-cols-2 grid w-full gap-4">
      <TrayFrame label="closed">
        <div className="tray-left w-40 bg-tone-50-surface p-4 shadow-lg">
...
function openAttributeShowcase() {
  return (
    <TrayFrame label="open attribute">
      <details open className="tray-right w-44 bg-tone-50-surface p-4 shadow-lg">
...
function toggleSlotShowcase() {
  return (
    <TrayFrame label='[data-slot="toggle"]:checked'>
      <div className="tray-left w-44 bg-tone-50-surface p-4 shadow-lg">
...
function TrayFrame({ children, label }: { children: ShowcaseJsxNode; label?: string }) {
  return (
    <ui-div className="space-y-2">
      {label && (
        <span className="inline-flex rounded bg-tone-50-surface/80 px-2 py-1 font-mono text-xs text-on-tone-50-surface/70">
          {label}
        </span>
      )}
      <ui-div className="relative h-40 overflow-hidden rounded-xl border border-tone-300-surface/40 bg-tone-100-surface/25">
        {children}
      </ui-div>
    </ui-div>
  )
...
function TrayContent({ title }: { title: string }) {
  return (
    <ui-div className="space-y-1">
      <p className="text-sm font-semibold text-on-tone-50-surface">{title}</p>
      <p className="text-xs text-on-tone-50-surface/65">
