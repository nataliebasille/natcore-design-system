...
} satisfies Record<ListControlValues['variant'], Record<ListControlValues['palette'], string>>
...
export function getListPlaygroundShowcase(values: ListControlValues) {
  ...
  return (
    <ui-div className="w-full rounded-2xl border border-tone-300-surface/50 bg-tone-50-surface p-4 text-sm text-on-tone-50-surface/80">
      <ListElement className={listClassName}>
        {items.map(([key, label]) => (
          <li key={key} data-slot="item" className={key === 'second' ? 'active' : undefined}>
            {label}
          </li>
        ))}
      </ListElement>
